// =====================================================
// MODELO ORIENTADO A GRAFOS - BASE DE DATOS DE PELÍCULAS
// Neo4j con Cypher
// =====================================================
...
MATCH (p:Pelicula {nombre: 'La vida es bella'}), (c:Critica {id: 2})
CREATE (p)-[:RECIBE_CRITICA]->(c);

MATCH (p:Pelicula {nombre: 'El Padrino'}), (c:Critica {id: 3})
CREATE (p)-[:RECIBE_CRITICA]->(c);

MATCH (p:Pelicula {nombre: 'Casablanca'}), (c:Critica {id: 4})
CREATE (p)-[:RECIBE_CRITICA]->(c);

MATCH (p:Pelicula {nombre: 'Titanic'}), (c:Critica {id: 5})
CREATE (p)-[:RECIBE_CRITICA]->(c);

MATCH (p:Pelicula {nombre: 'Forrest Gump'}), (c:Critica {id: 6})
CREATE (p)-[:RECIBE_CRITICA]->(c);

MATCH (p:Pelicula {nombre: 'Pulp Fiction'}), (c:Critica {id: 7})
CREATE (p)-[:RECIBE_CRITICA]->(c);

MATCH (p:Pelicula {nombre: 'El Rey León'}), (c:Critica {id: 8})
CREATE (p)-[:RECIBE_CRITICA]->(c);

// 10. CONSULTAS DE NEGOCIO
// =====================================================

// A) Total de salarios pagados a actores de "Cinema Paradiso"
// dirigida por "Giuseppe Tornatore"
MATCH (d:Director {nombre: 'Giuseppe Tornatore'})-[:DIRIGE]->(p:Pelicula {nombre: 'Cinema Paradiso'})<-[r:ACTUA_EN]-(a:Actor)
RETURN p.nombre AS pelicula,
       d.nombre AS director,
       a.nombre AS actor,
       r.salario AS salario,
       sum(r.salario) AS total_salarios;

// B) Premios recibidos por "Cinema Paradiso" con ranking
MATCH (d:Director {nombre: 'Giuseppe Tornatore'})-[:DIRIGE]->(p:Pelicula {nombre: 'Cinema Paradiso'})-[rp:RECIBE_PREMIO]->(pr:Premio)
RETURN p.nombre AS pelicula,
       p.ranking AS ranking,
       pr.nombre AS premio,
       pr.lugar_certamen AS lugar_certamen,
       pr.tipo_certamen AS tipo_certamen,
       pr.fecha AS fecha_premio
ORDER BY p.ranking DESC;

// C) Total de aportes económicos de "Franco Cristaldi"
MATCH (prod:Productor {nombre: 'Franco Cristaldi'})-[r:PRODUCE]->(p:Pelicula)
RETURN prod.nombre AS productor,
       count(p) AS peliculas_producidas,
       sum(r.aportacion_economica) AS total_aportes;

// D) Críticas de "Cinema Paradiso" entre fechas específicas
MATCH (d:Director {nombre: 'Giuseppe Tornatore'})-[:DIRIGE]->(p:Pelicula {nombre: 'Cinema Paradiso'})-[:RECIBE_CRITICA]->(c:Critica)
WHERE c.fecha >= date('1990-08-15') AND c.fecha <= date('1990-08-30')
RETURN p.nombre AS pelicula,
       c.medio AS medio,
       c.fecha AS fecha_critica,
       c.autor AS autor,
       p.ranking AS ranking
ORDER BY c.fecha DESC;

// E) Todas las personas involucradas en "Cinema Paradiso"
MATCH (p:Pelicula {nombre: 'Cinema Paradiso'})
OPTIONAL MATCH (a:Actor)-[:ACTUA_EN]->(p)
OPTIONAL MATCH (d:Director)-[:DIRIGE]->(p)
OPTIONAL MATCH (prod:Productor)-[:PRODUCE]->(p)
WITH p, 
     collect(DISTINCT {
         rol: 'Actor',
         nombre: a.nombre,
         edad_actual: duration.between(a.fecha_nacimiento, date()).years,
...
UNWIND (actores + directores + productores) AS persona
RETURN persona.rol AS rol,
       persona.nombre AS nombre,
       persona.edad_actual AS edad_actual,
       persona.estado_civil AS estado_civil,
